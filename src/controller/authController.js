...
// ========================================== conditions for authors data validation ====================================

        if(!Object.keys(data).length) { return res.status(400).send({ status: false, msg: "You must enter data." }) }  //data required
        ...
        if(!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(data.email.trim()))) { return res.status(400).send({ status: false, msg: "Enter a valid email address." }) }//used regex for email validation
